          ...
          <div class="mock-main">
            ...
            <div class="mock-chart">
              <svg viewBox="0 0 600 80">
                <path d="M0,60 Q80,40 160,55 T320,20 T480,35 T600,5" fill="none" stroke="#fff" stroke-width="2" opacity="0.15"/>
                <path d="M0,60 Q80,40 160,55 T320,20 T480,35 T600,5 V80 H0 Z" fill="url(#chartGrad)" opacity="0.06"/>
                <defs><linearGradient id="chartGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#fff"/><stop offset="100%" stop-color="transparent"/></linearGradient></defs>
              </svg>
            </div>
          </div>
        </div>
    ...
    <div class="plans-grid" class:visible={visibleSections['pricing']}>
      {#each plans as plan, i}
        <div class="plan-card" class:featured={plan.highlight} style="animation-delay: {i * 100}ms">
          ...
          <a href="/cadastro" class="plan-cta" class:plan-cta-featured={plan.highlight}>
            {plan.cta}
          </a>
...
<style>
  ...
  /* ===== NAV ===== */
  .nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; backdrop-filter: blur(20px); background: rgba(10,10,10,0.85); border-bottom: 1px solid rgba(255,255,255,0.06); }
  ...
  .nav-logo { height: auto; width: 120px; object-fit: contain; }
  .nav-links { display: flex; gap: 2.5rem; }
  .nav-links a { color: #666; text-decoration: none; font-size: 0.85rem; font-weight: 500; transition: color 0.2s; letter-spacing: -0.2px; }
  .nav-links a:hover { color: #fff; }
  .nav-actions { display: flex; align-items: center; gap: 1.25rem; }
  .link-login { color: #888; text-decoration: none; font-size: 0.85rem; font-weight: 500; }
  .link-login:hover { color: #fff; }
  .btn-nav { background: #fff; color: #000; border: none; padding: 0.55rem 1.2rem; border-radius: 8px; font-weight: 700; font-size: 0.82rem; cursor: pointer; font-family: inherit; transition: all 0.2s; text-decoration: none; }
  ...
  .grid-overlay { position: absolute; inset: 0; background-image: linear-gradient(rgba(255,255,255,0.015) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.015) 1px, transparent 1px); background-size: 80px 80px; mask-image: radial-gradient(ellipse 60% 50% at 50% 40%, black 20%, transparent 80%); }

  .hero-content { position: relative; z-index: 2; max-width: 750px; }
  .hero-pill { display: inline-flex; align-items: center; gap: 8px; padding: 0.4rem 1rem; border: 1px solid rgba(255,255,255,0.08); color: #999; border-radius: 999px; font-size: 0.78rem; font-weight: 500; margin-bottom: 2rem; background: rgba(255,255,255,0.02); letter-spacing: 0.2px; }
  .pill-dot { width: 5px; height: 5px; background: #fff; border-radius: 50%; animation: pulse-dot 2s infinite; }
  @keyframes pulse-dot { 0%, 100% { opacity: 1; } 50% { opacity: 0.3; } }

  h1 { font-size: 4.2rem; font-weight: 900; line-height: 1.02; letter-spacing: -3px; margin: 0 0 1.5rem; color: #fff; }
  .hero-sub { color: #666; font-size: 1.1rem; line-height: 1.7; max-width: 550px; margin: 0 auto 2.5rem; letter-spacing: -0.2px; }

  .hero-cta { display: flex; flex-direction: column; align-items: center; gap: 0.75rem; margin-bottom: 3rem; }
  .btn-primary-lg { display: inline-flex; align-items: center; gap: 10px; background: #fff; color: #000; border: none; padding: 0.9rem 2rem; border-radius: 10px; font-size: 0.95rem; font-weight: 700; cursor: pointer; font-family: 'Inter', sans-serif; transition: all 0.2s; letter-spacing: -0.3px; text-decoration: none; }
  .btn-primary-lg:hover { background: #e5e5e5; transform: translateY(-1px); }
  .btn-arrow { font-size: 1.1rem; transition: transform 0.2s; }
  .btn-primary-lg:hover .btn-arrow { transform: translateX(3px); }
  .hero-note { color: #444; font-size: 0.78rem; font-weight: 500; }

  .hero-proof { display: flex; align-items: center; gap: 12px; justify-content: center; }
  .proof-avatars { display: flex; }
  .proof-av { width: 30px; height: 30px; border-radius: 50%; background: #1a1a1a; border: 2px solid #0A0A0A; color: #888; font-size: 0.55rem; font-weight: 700; display: flex; align-items: center; justify-content: center; margin-left: -6px; }
  .proof-av:first-child { margin-left: 0; }
  .proof-text { color: #555; font-size: 0.82rem; }
  .proof-text strong { color: #999; }

  /* Hero Mockup */
  .hero-mockup { position: relative; z-index: 2; max-width: 850px; width: 100%; margin: 3rem auto 0; perspective: 1200px; }
  .mockup-window { background: #111; border: 1px solid #1a1a1a; border-radius: 14px; overflow: hidden; transform: rotateX(3deg); box-shadow: 0 50px 100px rgba(0,0,0,0.5); }
  .mockup-bar { display: flex; align-items: center; gap: 12px; padding: 10px 14px; background: #0d0d0d; border-bottom: 1px solid #1a1a1a; }
  .bar-dots { display: flex; gap: 5px; }
  .bar-dots span { width: 8px; height: 8px; border-radius: 50%; background: #1a1a1a; }
  .bar-url { color: #333; font-size: 0.65rem; font-weight: 500; margin-left: auto; }
  .mockup-screen { display: flex; height: 280px; }
  .mock-sidebar { width: 44px; background: #0d0d0d; border-right: 1px solid #1a1a1a; padding: 10px 0; display: flex; flex-direction: column; align-items: center; gap: 6px; }
  .ms-brand { margin-bottom: 10px; }
  .ms-logo-img { width: 20px; height: 20px; object-fit: contain; border-radius: 3px; }
  .ms-item { width: 24px; height: 24px; border-radius: 5px; background: #161616; }
  .ms-item.active { background: #1a1a1a; border: 1px solid #222; }
  .mock-main { flex: 1; padding: 14px; display: flex; flex-direction: column; gap: 10px; }
  .mock-topbar { height: 6px; width: 100%; background: rgba(255,255,255,0.03); border-radius: 3px; }
  .mock-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; }
  .mock-card { background: #161616; border: 1px solid #1a1a1a; border-radius: 8px; padding: 12px; display: flex; flex-direction: column; gap: 6px; }
  .mc-label { height: 4px; width: 40%; background: #222; border-radius: 2px; }
  .mc-val { height: 12px; width: 60%; background: #252525; border-radius: 3px; }
  .mc-white { background: #fff; border: none; }
  .mc-label.dark { background: #ccc; } .mc-val.dark { background: #aaa; }
  .mock-chart { flex: 1; padding: 8px; }
  ...
  .logos-bar p { color: #333; font-size: 0.7rem; font-weight: 600; text-transform: uppercase; letter-spacing: 3px; margin: 0 0 1rem; }
  .logos-scroll { display: flex; justify-content: center; gap: 2rem; flex-wrap: wrap; }
  .logos-scroll span { color: #444; font-size: 0.85rem; font-weight: 500; white-space: nowrap; }

  /* ===== SECTIONS SHARED ===== */
  .section-header { text-align: center; margin-bottom: 3rem; }
  .section-tag { color: #555; font-size: 0.68rem; font-weight: 700; text-transform: uppercase; letter-spacing: 4px; margin: 0 0 0.75rem; }
  .section-header h2 { font-size: 2.5rem; font-weight: 900; line-height: 1.08; letter-spacing: -1.5px; margin: 0 0 1rem; }
  .section-sub { color: #666; font-size: 1rem; margin: 0 auto; max-width: 500px; line-height: 1.6; }

  /* Scroll animation */
  .features-grid, .steps, .test-grid, .plans-grid, .faq-list { opacity: 0; transform: translateY(25px); transition: opacity 0.6s ease, transform 0.6s ease; }
  .visible { opacity: 1; transform: translateY(0); }

  /* ===== FEATURES ===== */
  .features { padding: 6rem 2rem; max-width: 1100px; margin: 0 auto; }
  .features-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1px; background: rgba(255,255,255,0.04); border-radius: 16px; overflow: hidden; border: 1px solid rgba(255,255,255,0.04); }
  .feature-card { background: #0e0e0e; padding: 2.25rem; transition: background 0.3s; }
  .feature-card:hover { background: #131313; }
  .fc-icon { font-size: 1.4rem; display: block; margin-bottom: 1.25rem; }
  .feature-card h3 { font-size: 1.05rem; font-weight: 800; margin: 0 0 0.5rem; letter-spacing: -0.3px; }
  .feature-card p { color: #555; font-size: 0.88rem; line-height: 1.6; margin: 0; }
  ...
  .step-num { font-size: 2.5rem; font-weight: 900; color: #222; margin-bottom: 1rem; }
  .step-content h3 { font-size: 1.05rem; font-weight: 800; margin: 0 0 0.5rem; letter-spacing: -0.3px; }
  .step-content p { color: #555; font-size: 0.88rem; line-height: 1.5; margin: 0; }
  ...
  .test-card { background: #0e0e0e; border: 1px solid #161616; border-radius: 14px; padding: 2rem; transition: border-color 0.3s; }
  .test-card:hover { border-color: #222; }
  .test-stars { color: #fff; font-size: 0.75rem; margin-bottom: 1rem; letter-spacing: 2px; opacity: 0.3; }
  .test-text { color: #888; font-size: 0.92rem; line-height: 1.65; margin: 0 0 1.5rem; }
  .test-author { display: flex; align-items: center; gap: 10px; }
  .test-av { width: 34px; height: 34px; background: #161616; border: 1px solid #1e1e1e; color: #666; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.65rem; font-weight: 700; flex-shrink: 0; }
  .test-author strong { display: block; color: #ccc; font-size: 0.82rem; }
  .test-author span { color: #444; font-size: 0.72rem; }

  /* ===== PRICING ===== */
  .pricing { padding: 6rem 2rem; max-width: 1100px; margin: 0 auto; }
  .billing-toggle { display: inline-flex; background: #111; border: 1px solid #1a1a1a; border-radius: 10px; padding: 3px; margin-top: 1.5rem; }
  .billing-toggle button { background: transparent; border: none; color: #555; padding: 0.55rem 1.5rem; border-radius: 8px; font-size: 0.82rem; font-weight: 600; cursor: pointer; font-family: inherit; transition: all 0.2s; display: inline-flex; align-items: center; gap: 6px; }
  .billing-toggle button.active { background: #1a1a1a; color: #fff; }
  .save-badge { background: #fff; color: #000; font-size: 0.6rem; font-weight: 800; padding: 2px 6px; border-radius: 4px; }

  .plans-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; align-items: start; }
  .plan-card { background: #0e0e0e; border: 1px solid #161616; border-radius: 16px; padding: 2rem; position: relative; transition: border-color 0.3s; }
  .plan-card:hover { border-color: #222; }
  .plan-card.featured { background: #111; border-color: #333; }
  .plan-badge { position: absolute; top: -11px; left: 50%; transform: translateX(-50%); background: #fff; color: #000; font-size: 0.65rem; font-weight: 800; padding: 4px 12px; border-radius: 20px; white-space: nowrap; }

  .plan-header { margin-bottom: 1.5rem; }
  .plan-header h3 { font-size: 1.3rem; font-weight: 800; margin: 0 0 0.25rem; letter-spacing: -0.5px; }
  .plan-desc { color: #555; font-size: 0.82rem; margin: 0; }

  .plan-price { margin-bottom: 1.5rem; display: flex; align-items: baseline; gap: 4px; flex-wrap: wrap; }
  .price-big { font-size: 2.8rem; font-weight: 900; letter-spacing: -2px; }
  .price-period { color: #444; font-size: 0.85rem; font-weight: 500; }
  .price-original { color: #333; font-size: 0.78rem; text-decoration: line-through; margin-left: 8px; }

  .plan-cta { display: block; width: 100%; padding: 0.8rem; background: #161616; border: 1px solid #1e1e1e; color: #ccc; border-radius: 10px; font-size: 0.9rem; font-weight: 700; cursor: pointer; font-family: inherit; transition: all 0.2s; margin-bottom: 1.5rem; text-decoration: none; text-align: center; box-sizing: border-box; }
  .plan-cta:hover { border-color: #333; background: #1a1a1a; color: #fff; }
  .plan-cta-featured { background: #fff; color: #000; border-color: #fff; }
  .plan-cta-featured:hover { background: #e5e5e5; border-color: #e5e5e5; }

  .plan-features { display: flex; flex-direction: column; gap: 0.55rem; }
  .pf-item { font-size: 0.82rem; color: #888; display: flex; align-items: center; gap: 8px; }
  .pf-item.disabled { color: #333; }
  .pf-check { color: #fff; font-weight: 700; font-size: 0.75rem; opacity: 0.5; }
  .pf-x { color: #2a2a2a; font-weight: 700; font-size: 0.75rem; }

  /* ===== FAQ ===== */
  .faq-section { padding: 6rem 2rem; max-width: 700px; margin: 0 auto; }
  .faq-list { display: flex; flex-direction: column; gap: 0.4rem; }
  .faq-item { background: #0e0e0e; border: 1px solid #161616; border-radius: 12px; padding: 1.15rem 1.4rem; width: 100%; text-align: left; cursor: pointer; font-family: inherit; color: inherit; transition: border-color 0.2s; }
  ...
  .faq-q span:first-child { font-size: 0.92rem; font-weight: 600; color: #ccc; }
  .faq-arrow { font-size: 1.1rem; color: #555; font-weight: 300; flex-shrink: 0; }
  .faq-a { color: #666; font-size: 0.88rem; line-height: 1.6; margin: 1rem 0 0; padding-top: 1rem; border-top: 1px solid #161616; }

  /* ===== FINAL CTA ===== */
  .final-cta { padding: 8rem 2rem; text-align: center; position: relative; }
  .cta-content { position: relative; z-index: 2; }
  .cta-content h2 { font-size: 2.8rem; font-weight: 900; letter-spacing: -1.5px; margin: 0 0 1rem; line-height: 1.08; }
  .cta-content p { color: #555; font-size: 1rem; margin: 0 0 2rem; }

  /* ===== FOOTER ===== */
  .footer { border-top: 1px solid rgba(255,255,255,0.04); padding: 3.5rem 2rem 1.5rem; }
  .footer-inner { max-width: 1100px; margin: 0 auto; display: flex; justify-content: space-between; gap: 3rem; margin-bottom: 3rem; }
  .footer-brand p { color: #333; font-size: 0.82rem; margin: 0.75rem 0 0; }
  .footer-links { display: flex; gap: 4rem; }
  .footer-col { display: flex; flex-direction: column; gap: 0.5rem; }
  .footer-col h4 { color: #444; font-size: 0.65rem; text-transform: uppercase; letter-spacing: 2px; font-weight: 700; margin: 0 0 0.4rem; }
  .footer-col a { color: #555; text-decoration: none; font-size: 0.82rem; transition: color 0.2s; }
  .footer-col a:hover { color: #fff; }
  .footer-bottom { max-width: 1100px; margin: 0 auto; padding-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.04); }
  .footer-bottom span { color: #2a2a2a; font-size: 0.75rem; }
